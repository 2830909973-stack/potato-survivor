...
export class GameOverUI {
  ...
  show(wave: number, level: number, kills: number, bossKills: number, peakAlive: number, materialsEarned: number, onRestart: () => void, onMenu: () => void) {
    ...
    const bg = this.scene.add.rectangle(W / 2, H / 2, 480, 400, 0x000000, 0.88).setOrigin(0.5);
    this.scene.add.text(W / 2, H / 2 - 180, "游戏结束", { fontSize: "36px", color: "#f44", fontStyle: "bold" }).setOrigin(0.5);
    this.scene.add.text(W / 2, H / 2 - 140, `波次 ${wave} · 等级 ${level}`, { fontSize: "18px", color: "#ff0" }).setOrigin(0.5);
    ...
    stats.forEach((s, i) => {
      this.scene.add.text(W / 2, H / 2 - 90 + i * 28, s, { fontSize: "15px", color: "#aaa" }).setOrigin(0.5);
    });
